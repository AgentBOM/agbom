...
@cli.command()
@click.option(
    "--path", type=click.Path(exists=True), default=".", help="Local path to scan"
)
...
@click.option(
    "--frameworks",
    default="langchain-py,langchain-ts",
...
@click.option(
    "--strict/--no-strict", default=True, help="Strict mode (literal tool arrays)"
)
@click.option(
    "--search/--no-search",
    default=True,
...
@click.option(
    "--search-keywords",
    help="Custom search keywords (comma-separated, only applies to --org with --search)",
)
@click.option(
    "--search-languages",
    help="Filter by languages (comma-separated, e.g., Python,TypeScript)",
)
        ...
        with Progress(
            ...
            if scan_type == "org":
                task = progress.add_task(
                    f"Scanning organization {scan_target}...", total=None
                )
                ...
                task = progress.add_task(
                    f"Scanning repository {scan_target}...", total=None
                )
...
def validate(file):
    ...
    try:
        ...
        console.print("✅ [green]Valid Agent BOM file[/green]")
...
def rate_limit():
    ...
    if not client.token:
        console.print(
            "[yellow]⚠️  No GitHub token found. Set GITHUB_ACCESS_TOKEN for authenticated requests.[/yellow]"
        )
    ...
    try:
        ...
        if search.get("remaining", 1) == 0:
            ...
            console.print(
                "   Tip: Use --search with smart_mode (default) to reduce API calls."
            )
        else:
