...
class LangChainTypeScriptDetector(BaseDetector):
    ...
    def _extract_agent_info(self, content: str, file_path: Path) -> Optional[dict]:
        ...
        executor_pattern = (
            r"(?:const|let|var)\s+(\w+)\s*=\s*new\s+AgentExecutor\s*\(\s*\{([^}]+)\}"
        )
        ...
        if match:
            ...
            info["tools"] = self._extract_tools_from_constructor(
                constructor_body, content, file_path
            )

        # Pattern 2: New createReactAgent pattern
        if not agent_found:
            react_agent_pattern = (
                r"(?:const|let|var)\s+(\w+)\s*=\s*createReactAgent\s*\(\s*\{([^}]+)\}"
            )
    ...
    def _extract_tools_from_constructor(
        self, constructor_body: str, content: str, file_path: Path
    ) -> List[ToolInfo]:
        ...
        if tools_ref_match:
            ...
            var_pattern = (
                rf"(?:const|let|var)\s+{re.escape(tools_var_name)}\s*=\s*\[([^\]]*)\]"
            )
    ...
    def _extract_tools_from_factory(
        ...
        for match in re.finditer(tool_factory_pattern, content):
            ...
            tool_info = self._find_tool_factory_definition(
                factory_name, content, file_path
            )
            if tool_info:
    ...
    def _extract_tools(
        self, tools_array: str, content: str, file_path: Path
    ) -> List[ToolInfo]:
    ...
    def _find_tool_factory_definition(
        self, factory_name: str, content: str, file_path: Path
    ) -> Optional[ToolInfo]:
        ...
        if import_match:
            ...
            tool_info = self._load_tool_from_import(
                factory_name, import_path, file_path
            )
            if tool_info:
                return tool_info
    ...
    def _extract_tool_from_factory_impl(
        self, factory_name: str, content: str
    ) -> Optional[ToolInfo]:
    ...
    def _extract_schema_params(self, content: str, schema_body: str) -> Dict[str, Any]:
        ...
        if extractor:
            try:
                ...
                logger.debug(
                    f"Error extracting schema with {extractor.get_library_name()}: {e}"
                )
                return {}
    ...
    def _find_tool_from_import(
        self, tool_var_name: str, content: str, file_path: Path
    ) -> Optional[ToolInfo]:
    ...
    def _load_tool_from_import(
        self, factory_name: str, import_path: str, current_file: Path
    ) -> Optional[ToolInfo]:
        ...
        if schema_match:
            ...
            if schema_ref_match:
                ...
                if var_def_match:
                    ...
                    tool_info.parameters = self._extract_schema_params(
                        content, schema_body
                    )
                else:
                    logger.debug(
                        f"Could not find schema variable {schema_var_name} in {file_path}"
                    )
