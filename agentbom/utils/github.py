...
class GitHubClient:
    ...
    def __init__(self, token: Optional[str] = None):
        ...
        self._min_search_interval = (
            2.5  # seconds between search requests (safety margin)
        )
    ...
    def clone_repo(
        self, repo: str, target_dir: Optional[Path] = None, shallow: bool = True
    ) -> Optional[Path]:
        """Clone a repository.
    ...
    def search_code_in_org(
        ...
        for keyword in keywords:
            ...
            if languages:
                ...
                for language in languages:
                    ...
                    logger.info(
                        f"  [{query_num}/{total_queries}] Searching: {language}..."
                    )
                    logger.debug(f"    Query: {lang_query}")
                    repos = self._search_code(lang_query, max_results_per_keyword)
                    logger.debug(
                        f"    Found {len(repos)} unique repo(s) for this query"
                    )
                    repos_found.update(repos)
            else:
                query_num += 1
                logger.info(
                    f"  [{query_num}/{total_queries}] Searching: '{keyword}'..."
                )
        ...
        logger.info(
            f"✓ Found {len(repo_list)} unique repository(ies) with potential agent/LLM code"
        )
        ...
        try:
            while len(repos) < max_results:
                ...
                if response.status_code == 403 or response.status_code == 429:
                    ...
                    if (
                        reset_time and (reset_time - time.time()) < 900
                        ...
                        logger.warning(
                            "Rate limit wait time too long. Returning partial results."
                        )
                        break
                ...
                if self._search_rate_limit_remaining is not None:
                    logger.debug(
                        f"Search API rate limit remaining: {self._search_rate_limit_remaining}"
                    )
                ...
                logger.debug(
                    f"GitHub returned {len(items)} items (total_count: {total_count})"
                )
    ...
    def filter_repos_by_language(
        self, repos: List[str], languages: List[str]
    ) -> List[str]:
